...
def create_tables():
    with get_db() as conn:
        ...
        ''')
        cursor.execute('SELECT COUNT(*) FROM cycle_state')
        if cursor.fetchone()[0] == 0:
            cursor.execute('INSERT INTO cycle_state (ID, CURRENT_USER_ID, NUMBERS_TAKEN) VALUES (?, ?, ?)', (1, 0, 0))
        cursor.execute('''
...
def get_cycle_state():
    """Возвращает текущего пользователя и количество взятых номеров в цикле."""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT CURRENT_USER_ID, NUMBERS_TAKEN FROM cycle_state WHERE ID = 1')
        result = cursor.fetchone()
        return result if result else (0, 0)

def update_cycle_state(user_id, numbers_taken):
    """Обновляет текущего пользователя и количество взятых номеров."""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('INSERT OR REPLACE INTO cycle_state (ID, CURRENT_USER_ID, NUMBERS_TAKEN) VALUES (?, ?, ?)',
                      (1, user_id, numbers_taken))
        conn.commit()
        print(f"[DEBUG] Cycle state обновлён: CURRENT_USER_ID={user_id}, NUMBERS_TAKEN={numbers_taken}")

def reset_cycle_state():
    """Сбрасывает состояние цикла."""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('INSERT OR REPLACE INTO cycle_state (ID, CURRENT_USER_ID, NUMBERS_TAKEN) VALUES (?, ?, ?)',
                      (1, 0, 0))
        conn.commit()
        print("[DEBUG] Cycle state сброшен")

def set_admin_state(admin_id, step, target_user_id=None, message_id=None):
    """Устанавливает состояние админа в таблице admin_state."""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('''
            INSERT OR REPLACE INTO admin_state (ADMIN_ID, STEP, TARGET_USER_ID, MESSAGE_ID)
            VALUES (?, ?, ?, ?)
        ''', (admin_id, step, target_user_id, message_id))
        conn.commit()
        print(f"[DEBUG] Установлено состояние для админа {admin_id}: STEP={step}, TARGET_USER_ID={target_user_id}, MESSAGE_ID={message_id}")

def get_admin_state(admin_id):
    """Получает состояние админа из таблицы admin_state."""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT STEP, TARGET_USER_ID, MESSAGE_ID FROM admin_state WHERE ADMIN_ID = ?', (admin_id,))
        result = cursor.fetchone()
...
def clear_admin_state(admin_id):
    ...
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('DELETE FROM admin_state WHERE ADMIN_ID = ?', (admin_id,))
        conn.commit()
        print(f"[DEBUG] Состояние админа {admin_id} очищено")

def set_numbers_per_cycle(user_id, count):
    """Устанавливает количество номеров за цикл для пользователя."""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('UPDATE users SET NUMBERS_PER_CYCLE = ? WHERE ID = ?', (count, user_id))
        if cursor.rowcount == 0:
            add_user(user_id)
            cursor.execute('UPDATE users SET NUMBERS_PER_CYCLE = ? WHERE ID = ?', (count, user_id))
        conn.commit()
        print(f"[DEBUG] Установлено количество номеров за цикл {count} для пользователя {user_id}")
...
def migrate_db():
    ...
    with get_db() as conn:
        ...
        if 'fa' not in columns:
            cursor.execute('ALTER TABLE users ADD COLUMN fa TEXT')
            print("[INFO] Столбец fa добавлен в таблицу users.")
        if 'NUMBERS_PER_CYCLE' not in columns:
            cursor.execute('ALTER TABLE users ADD COLUMN NUMBERS_PER_CYCLE INTEGER DEFAULT 1')
            print("[INFO] Столбец NUMBERS_PER_CYCLE добавлен в таблицу users.")
        

        cursor.execute('PRAGMA table_info(admin_state)')
        columns = [col[1] for col in cursor.fetchall()]
        if not columns:
            cursor.execute('''
            ...
            ''')
            print("[INFO] Таблица admin_state создана.")
        ...
        if 'fa' not in columns:
            cursor.execute('ALTER TABLE numbers ADD COLUMN fa TEXT')
            print("[INFO] Столбец fa добавлен в таблицу numbers.")
        if 'GROUP_MESSAGE_ID' not in columns:
            cursor.execute('ALTER TABLE numbers ADD COLUMN GROUP_MESSAGE_ID INTEGER')
            print("[INFO] Столбец GROUP_MESSAGE_ID добавлен в таблицу numbers.")
        ...
        if not columns:
            ...
            cursor.execute('INSERT INTO treasury (ID, BALANCE, AUTO_INPUT, CURRENCY) VALUES (?, ?, ?, ?)',
                          (1, 0, 0, 'USDT'))
        cursor.execute('PRAGMA table_info(cycle_state)')
        columns = [col[1] for col in cursor.fetchall()]
        if not columns:
            cursor.execute('''
            ...
            ''')
            cursor.execute('INSERT INTO cycle_state (ID, CURRENT_USER_ID, NUMBERS_TAKEN) VALUES (?, ?, ?)', (1, 0, 0))
            print("[INFO] Таблица cycle_state создана и инициализирована.")
        conn.commit()
...
def add_number(number, user_id, tg_group="1"):
    with get_db() as conn:
        cursor = conn.cursor()
        # Получаем 2FA из таблицы users
        cursor.execute('SELECT fa FROM users WHERE ID = ?', (user_id,))
        result = cursor.fetchone()
        fa_code = result[0] if result else None
        # Добавляем номер с 2FA
        cursor.execute('''
            INSERT INTO numbers (NUMBER, ID_OWNER, TAKE_DATE, SHUTDOWN_DATE, STATUS, TG_GROUP, SUBMIT_DATE, fa)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (number, user_id, '0', '0', 'ожидает', tg_group, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), fa_code))
        conn.commit()
        print(f"[DEBUG] Добавлен номер: {number}, ID_OWNER: {user_id}, STATUS: ожидает, TG_GROUP: {tg_group}, 2FA: {fa_code or 'не установлен'}")
...
def get_available_number(moderator_id):
    with get_db() as conn:
        cursor = conn.cursor()
        # Получаем текущее состояние цикла
        current_user_id, numbers_taken = get_cycle_state()
        
        # Получаем список пользователей с доступными номерами
        cursor.execute('''
        ...
        # Проверяем, есть ли текущий пользователь и не достигнут ли лимит
        if current_user_id and current_user_id in users:
            cursor.execute('SELECT NUMBERS_PER_CYCLE FROM users WHERE ID = ?', (current_user_id,))
            numbers_per_cycle = cursor.fetchone()[0] or 1
            if numbers_taken < numbers_per_cycle:
                # Пытаемся взять номер у текущего пользователя
                cursor.execute('''
                    SELECT n.NUMBER
                    FROM numbers n
                    LEFT JOIN users u ON n.ID_OWNER = u.ID
                ...
                if number:
                    update_cycle_state(current_user_id, numbers_taken + 1)
                    print(f"[DEBUG] Модератор {moderator_id} взял номер {number[0]} у пользователя {current_user_id} (взято: {numbers_taken + 1}/{numbers_per_cycle})")
                    return number[0]
        
        # Если текущий пользователь не подходит или лимит достигнут, выбираем нового
        reset_cycle_state()
        cursor.execute('''
            SELECT ID_OWNER 
            FROM numbers 
        ...
        if number:
            update_cycle_state(next_user_id, 1)
            cursor.execute('SELECT NUMBERS_PER_CYCLE FROM users WHERE ID = ?', (next_user_id,))
            numbers_per_cycle = cursor.fetchone()[0] or 1
            print(f"[DEBUG] Модератор {moderator_id} взял номер {number[0]} у пользователя {next_user_id} (взято: 1/{numbers_per_cycle})")
        ...
        print(f"[DEBUG] Доступные номера: {all_available}")
...
def toggle_afk_status(user_id):
    ...
    with get_db() as conn:
        ...
        if cursor.fetchone():
            print(f"[DEBUG] Пользователь {user_id} — модератор, АФК не применяется")
...
def update_last_activity(user_id):
    with get_db() as conn:
        ...
        if not result:
            cursor.execute(
                'INSERT OR IGNORE INTO users (ID, BALANCE, REG_DATE, IS_AFK, LAST_ACTIVITY) VALUES (?, ?, ?, ?, ?)',
                (user_id, 0.0, current_time, 0, current_time)
            )
            print(f"[DEBUG] Новый пользователь {user_id} добавлен с LAST_ACTIVITY={current_time}")

        # Просто обновляем время активности, не трогая IS_AFK
