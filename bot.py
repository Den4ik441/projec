...
def help_command(message):
    ...
    "• Настройка тарифов и АФК\n"
    "• Управление балансами пользователей\n"
    "• Отправка чеков (всем или выборочно)\n\n"
    "📱 <b>Работа с номерами</b>\n"
    "• <code>тг1</code> — взять номер в работу (от 1 до 70)\n"
    "• <code>слет +79991234567</code> — пометить номер как слетевший\n\n"
    "💰 <b>Выплаты</b>\n"
    "• Просмотр и обработка заявок на вывод\n"
    "• Отправка чеков через CryptoBot\n\n"
    f"📞 <b>Поддержка:</b> @{config.PAYOUT_MANAGER}"
        ...
        help_text = (
    "🛡 <b>Справка модератора</b>\n"
    "━━━━━━━━━━━━━━━\n"
    "📱 <b>Работа с номерами</b>\n"
    "• <code>тг1</code> — взять номер в работу (от 1 до 70)\n"
    "• <code>слет +79991234567</code> — пометить номер как слетевший\n\n"
    "🔙 <b>Возврат в меню</b>\n"
    "• Кнопка ниже или /start\n\n"
    f"📞 <b>Поддержка:</b> @{config.PAYOUT_MANAGER}"
        )
    else:
        ...
        help_text = (
    f"📚 <b>Справка {config.SERVICE_NAME}</b>\n"
    "━━━━━━━━━━━━━━━\n"
    f"🕒 <b>Режим работы:</b> <code>{config.WORK_TIME}</code>\n"
    f"💰 <b>Тариф:</b> {price}$ за номер (холд {hold_time} мин)\n\n"
    "💼 <b>Как это работает:</b>\n"
    "1️⃣ Сдайте номер 📱\n"
    "2️⃣ Дождитесь проверки 🔍\n"
    "3️⃣ Получите выплату 💰\n\n"
    "📋 <b>Возможности:</b>\n"
    "• Сдать номер\n"
    "• Удалить номер\n"
    "• Изменить номер\n"
    ...
    with db.get_db() as conn:
        ...
        try:
            ...
            text =     "✅ <b>Заявка одобрена</b>\n"
                        ...
                        "Теперь он имеет полный доступ к боту."
        except:
            text = f"    "✅ <b>Заявка одобрена</b>\n"
    "━━━━━━━━━━━━━━━\n"
    f"👤 Пользователь: <a href='tg://user?id={user_id}'>Профиль</a>\n"
    "Теперь он имеет полный доступ к боту.\n но уведомление не доставлено"
...
def reject_user_callback(call):
    ...
    "━━━━━━━━━━━━━━━\n"
    f"👤 Пользователь: <a href='tg://user?id={user_id}'>Профиль</a>\n"
    "Он сможет подать новый запрос через 15 минут.""
        except:
            text = f"    "❌ <b>Заявка отклонена</b>\n"
    "━━━━━━━━━━━━━━━\n"
    f"👤 Пользователь: <a href='tg://user?id={user_id}'>Профиль</a>\n"
...
def start_appeal_number(call):
    pending_appeals[call.from_user.id] = True
    markup = types.InlineKeyboardMarkup()
...
@bot.message_handler(func=lambda message: message.from_user.id in pending_appeals)
def process_appeal_number(message):
    user_id = message.from_user.id
    number = message.text.strip()

    # Убираем флаг, чтобы дальше он не реагировал на любое сообщение
    ...
    if not row:
        bot.send_message(message.chat.id, f"❌ <b>Ошибка</b>\n📱 {number} — не найден в базе.")
        return

    status = row[0]
    if status != "невалид":
        bot.send_message(message.chat.id, f"⚠️ <b>Отказ</b>\n📱 {number} — статус: {status}, апелляция невозможна.")
        return

    # Если всё ок — добавляем в апелляции
    with db_module.get_db() as conn:
        cursor = conn.cursor()
...
def process_delete_number(message, original_message_id):
    ...
    if not input_text:
        bot.send_message(
            chat_id,
            "⚠️ <b>Пустой ввод</b>\n━━━━━━━━━━━━━━━\n"
            "Вы не указали ни одного номера. Попробуйте снова 📝",
            parse_mode='HTML',
            disable_notification=True
        )
        start(message)
    ...
    try:
        with db_module.get_db() as conn:
            ...
            for number in numbers:
                ...
                cursor.execute(
                    'SELECT * FROM numbers WHERE ID_OWNER = ? AND NUMBER = ?',
                ...
                if not number_record:
                    results.append(
                        f"⚠️ <b>Не найден</b>\n"
                        f"📱 {normalized_number} — не найден или не принадлежит вам."
                    )
                    not_found_count += 1
                    continue

                cursor.execute(
                    'DELETE FROM numbers WHERE ID_OWNER = ? AND NUMBER = ?',
                    (user_id, normalized_number)
                )
                conn.commit()
                deleted_count += 1
                results.append(
                    f"🗑 <b>Удалено</b>\n"
                    f"📱 {normalized_number} — успешно удалён ✅"
                )
        ...
        summary += (
            ...
            f"⚠️ Некорректных: {invalid_count}\n"
        ...
        bot.send_message(
            chat_id,
            "❌ <b>Ошибка</b>\n━━━━━━━━━━━━━━━\n"
            "Произошёл сбой при удалении номеров. Попробуйте позже ⏳",
            parse_mode='HTML',
            disable_notification=True
        )
        start(message)
...
def process_old_number(message, original_message_id):
    ...
    if not old_number:
        bot.send_message(
            chat_id,
            "❌ Номер не может быть пустым. Попробуйте снова.",
            disable_notification=True  # Отключаем уведомление
        )
        start(message)
        return
    ...
    try:
        with db_module.get_db() as conn:
            ...
            if not number_record:
                bot.send_message(
                    chat_id,
                    f"f⚠️ <b>Номер не найден</b>\n"
                    f"📱 {normalized_old_number} — отсутствует в вашей базе",
                    disable_notification=True  # Отключаем уведомление
                )
                start(message)
                return
            ...
            msg = bot.send_message(
                chat_id,
                f"🔄 <b>Замена номера</b>\n"
                f"Старый: <code>{normalized_old_number}</code>\n\n"
                "Введите новый номер:\n"
                "📌 Пример: <code>+79991234567</code>",
                disable_notification=True  # Отключаем уведомление
            )
...
def process_new_number(message, original_message_id, old_number):
    ...
    if not new_number:
        bot.send_message(
            chat_id,
            "❌ Новый номер не может быть пустым. Попробуйте снова.",
            disable_notification=True  # Отключаем уведомление
        )
        start(message)
        return
    ...
    try:
        with db_module.get_db() as conn:
            ...
            bot.send_message(
                chat_id,
                f"✅ <b>Номер изменён</b>\n"
                f"📴 Был: <code>{old_number}</code>\n"
                f"📱 Стал: <code>{normalized_new_number}</code>",
                disable_notification=True  # Отключаем уведомление
            )
...
def reset_2fa(call):
    ...
    bot.send_message(
        call.message.chat.id,
        "🔓 <b>2FA сброшен</b>\n"
        "Теперь у номеров и аккаунта нет облачного пароля",
        parse_mode='HTML'
    )
    temp_message = bot.send_message(
        call.message.chat.id,
...
def show_pending_requests(call):
    ...
    text = f"📥 <b>Новые заявки на вступление</b> (страница {page}/{total_pages}):</b>\n\n"
    markup = types.InlineKeyboardMarkup()
...
def submit_number(call):
    ...
    # Проверяем, может ли пользователь сдавать номера
    with db.get_db() as conn:
        cursor = conn.cursor()
    ...
    msg = bot.send_message(
        ...
        (
            "📱 <b>Сдача номера</b>\n"
            "━━━━━━━━━━━━━━━\n"
            "Введите номер телефона, который хотите сдать.\n"
            "📌 Пример: <code>+79991234567</code>\n\n"
            "⚠️ Разрешены только российские номера.\n"
            f"💵 <b>Текущий тариф:</b> {price}$ | ⏱ Холд: {hold_time} мин"
        ),
        reply_markup=markup,
        parse_mode='HTML'
    )
...
def process_numbers(message):
    if not message or not message.text:
        ...
        bot.send_message(
            message.chat.id,
            "❌ <b>Пожалуйста, отправьте номера текстом!</b>",
            reply_markup=markup,
            parse_mode='HTML',
            disable_notification=True
        )
    ...
    numbers = [num.strip() for num in message.text.strip().split('\n') if num.strip()]
    if not numbers:
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("📱 Попробовать снова", callback_data="submit_number"))
        markup.add(types.InlineKeyboardButton("🔙 Вернуться в меню", callback_data="back_to_main"))
        bot.send_message(
            message.chat.id,
            "❌ <b>Вы не указали ни одного номера!</b>",
            reply_markup=markup,
            parse_mode='HTML',
            disable_notification=True
        )
    ...
    if not valid_numbers:
        ...
        if invalid_numbers:
            response_text += "📛 <b>Неверный формат:</b>\n" + "\n".join(invalid_numbers)
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("📱 Попробовать снова", callback_data="submit_number"))
        markup.add(types.InlineKeyboardButton("🔙 Вернуться в меню", callback_data="back_to_main"))
        bot.send_message(
    ...
    try:
        ...
        response_text = "📊 <b>Результаты добавления номеров</b>\n━━━━━━━━━━━━━━━\n"
        if success_count > 0:
            response_text += f"✅ <b>Успешно добавлено:</b> {success_count}\n📱 {', '.join(successfully_added)}\n\n"
        if already_exists > 0:
            response_text += f"⚠️ <b>Уже существуют:</b> {already_exists}\n"
        if restricted_status_count > 0:
            response_text += f"🚫 <b>Запрещённый статус:</b> {restricted_status_count}\n📱 {', '.join(restricted_status_numbers)}\n"
        if invalid_numbers:
            response_text += f"❌ <b>Неверный формат:</b>\n" + "\n".join(invalid_numbers) + "\n"
        ...
        response_text = (
            "⚠️ <b>Ошибка</b>\n"
            "━━━━━━━━━━━━━━━\n"
            "Произошла ошибка при добавлении номеров. Попробуйте снова позже."
        )
    ...
    try:
        ...
        if owner:
            ...
            if afk_locked and afk_locked[0] == 1:
                bot.send_message(
                    call.message.chat.id,
                    f"📱 ТГ {tg_number}\n📱 Номер: {number}\n🔒Администратор запретил ему сдавать номера:",
                    parse_mode='HTML'
                )
                return
            ...
            try:
                markup = types.InlineKeyboardMarkup()
                markup.add(
                    types.InlineKeyboardButton("❌ Не валид",
                        callback_data=f"mark_invalid_{number}_{group_chat_id}_{tg_number}")
                )
                msg = bot.send_message(
                    owner_id,
                    "=================\n"
                ...
                markup.add(
                    types.InlineKeyboardButton("❌ Не валид",
                        callback_data=f"moderator_invalid_{number}_{tg_number}_{owner_id}")
                )
                bot.send_message(
                    call.message.chat.id,
                    f"📱 <b>ТГ {tg_number}</b>\n"
    ...
    try:
        ...
        if action == "confirm_code":
            ...
            with db.get_db() as conn:
                ...
                cursor.execute('UPDATE numbers SET STATUS = ? WHERE NUMBER = ?', ('невалид', number))
                conn.commit()
                print(f"[DEBUG] Номер {number} изменен на невалид")
            ...
            with db.get_db() as conn:
                ...
                cursor.execute('UPDATE numbers SET STATUS = ? WHERE NUMBER = ?', ('невалид', number))
                conn.commit()
                print(f"[DEBUG] Номер {number} изменен статус на невалид")
...
def number_active(call):
    try:
        ...
        bot.edit_message_text(
            f"📱 <b>ТГ {tg_number}</b>\n"
            f"✅ Номер {number} подтверждён в {current_time}.\n"
            f"📱 Код: {verification_code}\n"
            f"🔒 2FA: {fa}",
            call.message.chat.id,
            call.message.message_id,
