                ...
                if group_message_id:
                    try:
                        ...
                        with db_module.get_db() as conn:
                            ...
                            cursor.execute('UPDATE numbers SET GROUP_MESSAGE_ID = ? WHERE NUMBER = ?', (msg.message_id, number))
                            conn.commit()
                            print(f"[DEBUG] Отправлено новое сообщение в группу: number={number}, new_group_message_id={msg.message_id}")
                else:
                    msg = bot.send_message(
                        group_chat_id,
                        (
                    ...
                    with db_module.get_db() as conn:
                        ...
                        cursor.execute('UPDATE numbers SET GROUP_MESSAGE_ID = ? WHERE NUMBER = ?', (msg.message_id, number))
                        conn.commit()
                        print(f"[DEBUG] Отправлено новое сообщение в группу: number={number}, group_message_id={msg.message_id}")
