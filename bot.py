...
def handle_delete_number(call):
    ...
    msg = bot.send_message(chat_id, "📞 Впишите номера для удаления (по одному в строке, например:\n+79891234567\n+79091234567):")
...
def process_delete_number(message, original_message_id):
    ...
    if not input_text:
        bot.send_message(chat_id, "❌ Ввод не может быть пустым. Попробуйте снова.")
        start(message)
        return
    ...
    try:
        with db_module.get_db() as conn:
            ...
            for number in numbers:
                ...
                if not normalized_number or not re.match(r'^\+7\d{10}$', normalized_number):
                    results.append(f"❌ Номер {number}: некорректный формат. Используйте российский номер, например, +79991234567.")
                    invalid_count += 1
                    continue
        ...
        bot.send_message(chat_id, "❌ Произошла ошибка при удалении номеров. Попробуйте позже.")
        start(message)
#ИЗММЕНИТЬ НОМЕР:
...
def calculate_amount_to_send(target_amount):
    ...
    amount_with_fee = target_amount / (1 - commission_rate)  # Делим на 0.97
    rounded_amount = round(amount_with_fee, 2)  # Округляем до 2 знаков после запятой
    
    # Проверяем, что после вычета комиссии получаем не меньше target_amount
    received_amount = rounded_amount * (1 - commission_rate)
    if received_amount < target_amount:
        rounded_amount += 0.01  # Увеличиваем на минимальную единицу, если сумма недостаточна
...
def user_details(call):
    ...
    with db.get_db() as conn:
        ...
        invalid_numbers = sum(1 for num in numbers if num[0] == 'не валид')  # Сколько не валидных
...
def show_group_stats(call):
    ...
    with db.get_db() as conn:
        ...
        # Подсчёт номеров для группы (включая автоматически подтверждённые)
        cursor.execute('''
            SELECT COUNT(*) 
            FROM numbers n
...
def show_all_numbers(call):
    ...
    if not numbers:
        ...
        for number, status, take_date, shutdown_date, owner_id, confirmed_by_moderator_id, group_chat_id, tg_number in numbers:
            ...
            numbers_text += (
                f"📱 Номер: <code>{number}</code>\n"
                f"👤 Владелец:  tg://openmessage?user_id={owner_id}\n"
                f"📊 Статус: {status}\n"
                f"🟢 Взято: {take_date_str}\n"
                f"🔴 Отстоял: {shutdown_date_str}\n"
                f"{moderator_info}\n"
                        ...
                        with db_module.get_db() as conn:
                            ...
                            try:
                                bot.edit_message_text(
                                    ...
                                    f"✅ Номер {number} автоматически подтверждён через 2 минуты.",
                                    data["chat_id"],
                                    data["message_id"],
                                    parse_mode='HTML'
                                )
...
def process_user_id_for_balance(message, original_chat_id, original_message_id):
    ...
    with db_module.get_db() as conn:
        ...
        if not user:
            ...
            return
        target_user_id, current_balance, username = user
    ...
    # Запрашиваем сумму
    username_display = f"@{username}" if username and username != "Не указан" else "Нет username"
    text = f"💸 <b>Уменьшение баланса для пользователя {target_user_id} ({username_display})</b>\n" \
           f"Текущий баланс: {current_balance} $\n" \
           f"На сколько вы хотите уменьшить баланс? (введите число в $):"
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="Gv"))
    ...
    try:
        ...
        bot.register_next_step_handler(msg, process_balance_reduction, target_user_id, original_chat_id, original_message_id, current_balance)
    except Exception as e:
        print(f"[ERROR] Не удалось обновить сообщение: {e}")
        msg = bot.send_message(
            original_chat_id,
        ...
        bot.register_next_step_handler(msg, process_balance_reduction, target_user_id, original_chat_id, msg.message_id, current_balance)

def process_balance_reduction(message, target_user_id, original_chat_id, original_message_id, current_balance):
    user_id = message.from_user.id
    ...
    # Списываем сумму с блокировкой для избежания конкурентного доступа
    with db_lock:
        with db_module.get_db() as conn:
            cursor = conn.cursor()
            # Повторно проверяем баланс перед списанием
            ...
            # Уменьшаем баланс, используя BALANCE = BALANCE - ?
            cursor.execute('UPDATE users SET BALANCE = BALANCE - ? WHERE ID = ?', (amount, target_user_id))
            # Получаем новый баланс для уведомлений
            cursor.execute('SELECT BALANCE FROM users WHERE ID = ?', (target_user_id,))
            new_balance = cursor.fetchone()[0]
            conn.commit()
            logging.info(f"[BALANCE] Баланс пользователя {target_user_id} уменьшен на {amount} $. Новый баланс: {new_balance} $")
    ...
    with db_module.get_db() as conn:
        ...
        user = cursor.fetchone()
        username_display = f"@{user[0]}" if user and user[0] and user[0] != "Не указан" else "Нет username"
    
    admin_message = (
        f"✅ Баланс пользователя {target_user_id} ({username_display}) уменьшен на {amount} $.\n"
    ...
    # Уведомляем пользователя
    user_message = (
        f"💸 Ваш баланс был уменьшен на {amount} $ администратором.\n"
        f"Текущий баланс: {new_balance} $"
    )
    try:
        bot.send_message(
        ...
        print(f"[ERROR] Не удалось уведомить пользователя {target_user_id}: {e}")
        bot.send_message(
            original_chat_id,
            f"⚠️ Не удалось уведомить пользователя {target_user_id} о снижении баланса: {e}",
            parse_mode='HTML'
        )
...
def handle_failed_number(message):
    ...
    # Проверка, зарегистрирована ли группа (для групповых чатов)
    if message.chat.type in ['group', 'supergroup']:
        group_ids = db_module.get_all_group_ids()
        if chat_id not in group_ids:
            bot.reply_to(message, "❌ Эта группа не зарегистрирована для выполнения команд!")
            return
