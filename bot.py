...
def help_command(message):
    ...
    with db_module.get_db() as conn:
        ...
        if request and request[1] == 1:
            bot.send_message(chat_id, f"🚫 Вы заблокированы. Обратитесь к поддержке: @{config.PAYOUT_MANAGER}", parse_mode='HTML')
            return
        if not request or request[0] != 'approved':
            bot.send_message(chat_id, "👋 Ваша заявка ожидает одобрения администратора.", parse_mode='HTML')
            return

    is_admin = user_id in config.ADMINS_ID
    is_moderator = db_module.is_moderator(user_id)
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔙 Меню", callback_data="back_to_main"))

    if is_admin:
        help_text = (
            "👑 <b>Админ: Справка</b>\n\n"
            "⚙️ <b>Админ-панель</b>:\n"
            "   - Управление заявками, АФК, ценами, балансами, чеками\n"
            "📱 <b>Номера</b>:\n"
            "   - <code>тг1</code> (1-70) — взять номер\n"
            "   - <code>слет +79991234567</code> — пометить слет\n"
            "💰 <b>Выплаты</b>: Обработка заявок, отправка чеков\n"
            "📊 <b>Статистика</b>: В профиле\n"
            f"📞 <b>Поддержка</b>: @{config.PAYOUT_MANAGER}"
        )
    elif is_moderator:
        help_text = (
            "🛡 <b>Модератор: Справка</b>\n\n"
            "📱 <b>Номера</b>:\n"
            "   - <code>тг1</code> (1-70) — взять номер\n"
            "   - <code>слет +79991234567</code> — пометить слет\n"
            "   - Подтверждение/отклонение через кнопки\n"
            f"📞 <b>Поддержка</b>: @{config.PAYOUT_MANAGER}"
        )
    else:
        with db_module.get_db() as conn:
            ...
            price, hold_time = cursor.fetchone() or (2.0, 5)

        help_text = (
            f"<b>📢 {config.SERVICE_NAME}: Справка</b>\n\n"
            f"<b>⏳ График</b>: {config.WORK_TIME}\n"
            f"<b>💼 Как работает?</b> Сдаёте номер — получаете {price}$ после {hold_time} мин.\n"
            "<b>📱 Действия</b>:\n"
            "   - Сдать/удалить/изменить номер\n"
            "   - Профиль: баланс, номера\n"
            "   - Вывод средств\n"
            "   - АФК-режим\n"
            f"<b>📞 Поддержка</b>: @{config.PAYOUT_MANAGER}"
        )
...
def queue_command(message):
    ...
    try:
        with db_module.get_db() as conn:
            ...
            if request and request[1] == 1:
                bot.send_message(chat_id, f"🚫 Вы заблокированы. Пишите в поддержку: @{config.PAYOUT_MANAGER}", parse_mode='HTML')
                logging.info(f"User {user_id} blocked, cannot access queue")
                return
            if not request or request[0] != 'approved':
                bot.send_message(chat_id, "👋 Ждите, пока вашу заявку одобрят!", parse_mode='HTML')
                logging.info(f"User {user_id} not approved, cannot access queue")
                return
            ...
            if not user_status:
                bot.send_message(chat_id, "❌ Ваш профиль не найден. Обратитесь в поддержку!", parse_mode='HTML')
                logging.error(f"User {user_id} not found in users table")
                return
            if user_status[0] == 1 or user_status[1] == 1:
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton("🔙 Меню", callback_data="back_to_main"))
                bot.send_message(
                    chat_id,
                    "📪 Вы в режиме АФК. Ваши номера не в очереди. Отключите АФК в профиле!",
                    parse_mode='HTML',
                    reply_markup=markup
                )
                logging.info(f"User {user_id} in AFK, cannot access queue")
                return

            # Проверяем номера пользователя
            cursor.execute('''
                SELECT NUMBER, SUBMIT_DATE 
                FROM numbers 
                WHERE ID_OWNER = ? AND STATUS = 'ожидает' AND TAKE_DATE = '0'
            ...
            if not user_number:
                ...
                bot.send_message(
                    chat_id,
                    "📪 У вас нет номеров в очереди. Сдайте номер, чтобы начать!",
                    parse_mode='HTML',
                    reply_markup=markup
                )
                logging.info(f"User {user_id} has no numbers in queue")
                return

            # Собираем список пользователей с номерами в очереди
            cursor.execute('''
            ...
            if not users:
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton("📱 Сдать ещё номер", callback_data="submit_number"))
                markup.add(types.InlineKeyboardButton("🔙 Меню", callback_data="back_to_main"))
                bot.send_message(
                    chat_id,
                    f"📱 <b>Ваш номер:</b> <code>{user_number[0]}</code>\n"
                    f"🎯 <b>Ваш номер в очереди:</b> 1\n"
                    f"⏳ <b>Номеров впереди:</b> 0\n"
                    f"📋 <b>Номеров в ожидании:</b> 1\n\n"
                    f"Ждите, ваш номер скоро проверят!(Очередь может изменяться)",
                    parse_mode='HTML',
                    reply_markup=markup
                )
                logging.info(f"User {user_id} is the only one in queue with number {user_number[0]}")
                return

            # Определяем последнего взятого пользователя
            cursor.execute('''
                SELECT ID_OWNER 
                FROM numbers 
            ...
            # Рассчитываем позицию в ротации
            ...
            for i in range(len(users)):
                ...
                cursor.execute('''
                    ...
                    AND u.IS_AFK = 0
                    AND u.AFK_LOCKED = 0
                    ORDER BY n.SUBMIT_DATE ASC
                    LIMIT 1
                ''', (owner_id,))
                number = cursor.fetchone()
                if number:
                    total_queue += 1
                    if owner_id == user_id and not user_position:
                        user_position = i + 1
                        numbers_ahead = i

            # Проверяем, если пользователь не попал в ротацию
            if not user_position:
                ...
                bot.send_message(
                    chat_id,
                    f"📱 <b>Ваш номер:</b> <code>{user_number[0]}</code>\n"
                    f"📪 Ваш номер не в активной очереди из-за ротации. Ждите своей очереди!",
                    parse_mode='HTML',
                    reply_markup=markup
                )
                logging.warning(f"User {user_id} number {user_number[0]} not in active rotation")
                return
            ...
            help_text = (
                f"📱 <b>Ваш номер:</b> <code>{user_number[0]}</code>\n"
                f"🎯 <b>Ваш номер в очереди:</b> {user_position}\n"
                f"⏳ <b>Номеров впереди:</b> {numbers_ahead}\n"
                f"📋 <b>Номеров в ожидании:</b> {total_queue}\n\n"
                f"Ждите, ваш номер скоро проверят!"
            )
            bot.send_message(chat_id, help_text, parse_mode='HTML', reply_markup=markup)
            logging.info(f"User {user_id} checked queue: position={user_position}, ahead={numbers_ahead}, total={total_queue}")

    except sqlite3.Error as e:
        logging.error(f"Database error in queue_command: {e}")
        bot.send_message(chat_id, "❌ Ошибка! Попробуйте позже.", parse_mode='HTML')
        return
...
def group_statistics(call):
    ...
    if "_" in call.data:
        try:
            # Извлекаем число после "group_statistics_" более надёжно
            page_parts = call.data.replace("group_statistics_", "").split("_")
            page = int(page_parts[0]) if page_parts and page_parts[0].isdigit() else 1
            print(f"[DEBUG] Извлечена страница из {call.data}: {page}")
        except (IndexError, ValueError):
            page = 1
            print(f"[DEBUG] Ошибка парсинга page из {call.data}, установлено значение по умолчанию: {page}")
    ...
    page = max(1, min(page, total_pages))
    print(f"[DEBUG] Общее количество групп: {len(groups)}, страниц: {total_pages}, запрошенная страница: {page}")
    ...
    # Генерация текста
    new_text = f"<b>📊 Список групп (страница {page}/{total_pages}):</b>\n\n"
    for group_id, group_name in page_groups:
        new_text += f"🏠 <b>{group_name}</b>\n"
        new_text += "────────────────────\n"
    ...
    new_markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="admin_panel"))
    new_markup.add(types.InlineKeyboardButton("🔙 Главное меню", callback_data="back_to_main"))

    # Проверка и редактирование сообщения
    try:
        # Проверяем, нужно ли редактировать (избегаем повторного вызова с той же страницей)
        if page != getattr(call, 'previous_page', None):
            bot.edit_message_text(new_text, call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=new_markup)
            print(f"[DEBUG] Успешно обновлено сообщение для страницы {page}")
            call.previous_page = page  # Сохраняем текущую страницу во временном атрибуте
        else:
            print(f"[DEBUG] Страница {page} уже отображается, редактирование пропущено")
            bot.answer_callback_query(call.id, "📌 Страница уже отображается.")
    except telebot.apihelper.ApiTelegramException as e:
        if e.result_json and e.result_json.get('description') == "Bad Request: message is not modified":
            print(f"[DEBUG] Сообщение для страницы {page} не изменено, редактирование пропущено")
            bot.answer_callback_query(call.id, "📌 Страница уже отображается.")
        else:
            print(f"[ERROR] Не удалось обновить сообщение для страницы {page}: {e}")
            bot.answer_callback_query(call.id, "❌ Ошибка при обновлении страницы. Попробуйте снова.")
    except Exception as e:
        print(f"[ERROR] Неизвестная ошибка при обновлении сообщения для страницы {page}: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка при обновлении страницы. Попробуйте снова.")
    ...
    while True:
        try:
            ...
            with db_module.get_db() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT NUMBER, GROUP_MESSAGE_ID, GROUP_CHAT_ID, TG_NUMBER, ID_OWNER, MODERATOR_ID, 
                           VERIFICATION_CODE, fa, TAKE_DATE
                    FROM numbers 
                    WHERE STATUS IN ('на проверке', 'taken') AND TAKE_DATE IS NOT NULL
                ''')
                numbers = cursor.fetchall()

                for number_data in numbers:
                    try:
                        number, group_message_id, group_chat_id, tg_number, owner_id, moderator_id, verification_code, fa, take_date = number_data
                        
                        # Если fa пустой — заменяем на "не установлен"
                        fa_display = fa if fa else "не установлен"
                        
                        # Проверка корректности TAKE_DATE
                        try:
                            take_date = datetime.strptime(take_date, "%Y-%m-%d %H:%M:%S")
                        except (ValueError, TypeError) as e:
                            print(f"[TIMEOUT_CHECK] Некорректный TAKE_DATE для номера {number}: {take_date}")
                            continue

                        elapsed_time = (current_time - take_date).total_seconds() / 60
                        print(f"[TIMEOUT_CHECK] Номер {number}, прошло времени: {elapsed_time:.2f} минут, TG: {tg_number or 'N/A'}")
                        ...
                        if elapsed_time >= 2:
                            ...
                            cursor.execute('''
                                UPDATE numbers 
                                SET STATUS = 'активен', TAKE_DATE = ?, VERIFICATION_CODE = NULL, 
                                    CONFIRMED_BY_MODERATOR_ID = NULL, GROUP_MESSAGE_ID = NULL
                                WHERE NUMBER = ?
                            ''', (current_date, number))
                            conn.commit()
                            ...
                            if group_message_id and group_chat_id:
                                try:
                                    ...
                                    print(f"[TIMEOUT_CHECK] Обновлено сообщение в группе {group_chat_id}")
                                except Exception as e:
                                    ...
                                    try:
                                        ...
                                        cursor.execute('UPDATE numbers SET GROUP_MESSAGE_ID = ? WHERE NUMBER = ?', (msg.message_id, number))
                                        conn.commit()
                                    except Exception as e:
                                        print(f"[TIMEOUT_CHECK] Не удалось отправить новое сообщение в группу {group_chat_id}: {e}")
...
def settingssss(data):
    ...
    markup.add(types.InlineKeyboardButton("📊 Изменить количество номеров за круг", callback_data="change_numbers_per_cycle"))
    markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="admin_panel"))
...
def admin_enable_afk(call):
    ...
    # Очистка данных номеров пользователя
    with db_module.get_db() as conn:
        cursor = conn.cursor()
        ...
        for number_tuple in numbers:
            ...
            cursor.execute('UPDATE numbers SET GROUP_MESSAGE_ID = NULL WHERE NUMBER = ?', (number,))
        conn.commit()
    
    # Обновляем статус AFK
    with db_module.get_db() as conn:
        cursor = conn.cursor()
...
def back_to_admin_settings(call):
    admin_id = call.from_user.id
    db_module.clear_admin_state(admin_id)  # 🛑 очищаем состояние
...
@bot.callback_query_handler(func=lambda call: call.data == "change_numbers_per_cycle")
def change_numbers_per_cycle(call):
    admin_id = call.from_user.id
    if admin_id not in config.ADMINS_ID:
        bot.answer_callback_query(call.id, "❌ У вас нет прав для изменения количества номеров за круг!")
        return
    
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="back_to_admin_settings"))

    msg = bot.edit_message_text(
        "📊 <b>Изменить количество номеров за круг</b>\n\n"
        "Введите ID пользователя или @username для установки количества номеров за цикл:",
        call.message.chat.id,
        call.message.message_id,
        parse_mode='HTML',
        reply_markup=markup
    )
    db_module.set_admin_state(admin_id, "awaiting_user_id", message_id=msg.message_id)
    bot.register_next_step_handler(msg, process_numbers_per_cycle_user_id)
    bot.answer_callback_query(call.id)


def process_numbers_per_cycle_user_id(message):
    admin_id = message.from_user.id
    step, _, _ = db_module.get_admin_state(admin_id)
    
    if step != "awaiting_user_id":
        print(f"[DEBUG] Неверное состояние для админа {admin_id}: {step}")
        return
    ...
    input_text = message.text.strip()
    print(f"[DEBUG] Введённый текст: '{input_text}'")

    target_user_id = None
    username = None
    if input_text.startswith('@'):
        username = input_text[1:]  # Убираем @ (например, Devshop19)
        print(f"[DEBUG] Обработка username: {username}")
    else:
        try:
            target_user_id = int(input_text)
            print(f"[DEBUG] Обработка user ID: {target_user_id}")
        except ValueError:
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="change_numbers_per_cycle"))
            bot.send_message(message.chat.id, "❌ Неверный формат. Введите числовой ID или @username.", reply_markup=markup)
            bot.register_next_step_handler(message, process_numbers_per_cycle_user_id)
            return
    ...
    if username:
        with db.get_db() as conn:
            ...
            if user:
                target_user_id = user[0]
                print(f"[DEBUG] Найден user ID {target_user_id} для username {username}")
            else:
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="change_numbers_per_cycle"))
                bot.send_message(message.chat.id, f"❌ Пользователь с @username '{username}' не найден.", reply_markup=markup)
                bot.register_next_step_handler(message, process_numbers_per_cycle_user_id)
                print(f"[DEBUG] Username {username} не найден в БД")
                return

    try:
        with db.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT USERNAME, NUMBERS_PER_CYCLE FROM users WHERE ID = ?', (target_user_id,))
            user = cursor.fetchone()
            if not user:
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="change_numbers_per_cycle"))
            ...
            username, numbers_per_cycle = user
            numbers_per_cycle = numbers_per_cycle or 1  # По умолчанию 1, если None
            print(f"[DEBUG] Пользователь {target_user_id}: USERNAME={username}, NUMBERS_PER_CYCLE={numbers_per_cycle}")
            ...
            msg = bot.send_message(
                ...
                f"👤 <b>User ID:</b> {target_user_id}\n"
                f"{username_text}"
                f"📊 <b>Текущее количество номеров за цикл:</b> {numbers_per_cycle}\n\n"
                "Введите новое количество номеров за цикл (целое число больше 0):",
                parse_mode='HTML',
                reply_markup=markup
            )
            # Обновляем состояние в базе данных
            db_module.set_admin_state(admin_id, "awaiting_count", target_user_id=target_user_id, message_id=msg.message_id)
            bot.register_next_step_handler(msg, process_numbers_per_cycle_count)
    except Exception as e:
        print(f"[ERROR] Ошибка при обработке пользователя {target_user_id}: {e}")
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="change_numbers_per_cycle"))
        bot.send_message(message.chat.id, "❌ Произошла ошибка при получении данных. Попробуйте позже.", reply_markup=markup)
        bot.register_next_step_handler(message, process_numbers_per_cycle_user_id)

def process_numbers_per_cycle_count(message):
    admin_id = message.from_user.id
    step, target_user_id, _ = db_module.get_admin_state(admin_id)
    ...
    try:
        ...
        if count < 1:
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="change_numbers_per_cycle"))
            bot.send_message(message.chat.id, "❌ Количество номеров должно быть больше 0!", reply_markup=markup)
            bot.register_next_step_handler(message, process_numbers_per_cycle_count)
            return
        
        db_module.set_numbers_per_cycle(target_user_id, count)
        ...
        bot.send_message(
            message.chat.id,
            f"✅ Количество номеров за цикл для пользователя {target_user_id} установлено: {count}",
            parse_mode='HTML',
            reply_markup=markup
        )
        print(f"[DEBUG] Установлено NUMBERS_PER_CYCLE={count} для пользователя {target_user_id}")
    except ValueError:
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="change_numbers_per_cycle"))
        bot.send_message(message.chat.id, "❌ Пожалуйста, введите корректное число!", reply_markup=markup)
        bot.register_next_step_handler(message, process_numbers_per_cycle_count)
    except Exception as e:
        print(f"[ERROR] Ошибка при установке количества номеров для пользователя {target_user_id}: {e}")
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 Назад", callback_data="change_numbers_per_cycle"))
        bot.send_message(message.chat.id, "❌ Произошла ошибка. Попробуйте снова.", reply_markup=markup)
        bot.register_next_step_handler(message, process_numbers_per_cycle_count)
...
#братья номеров для тг
def get_number_in_group(user_id, chat_id, message_id, tg_number):
    print(f"[DEBUG] get_number_in_group: user_id={user_id}, chat_id={chat_id}, tg_number={tg_number}")
    ...
    if number:
        with db_module.get_db() as conn:
            ...
            cursor.execute('''
                UPDATE numbers 
                SET TAKE_DATE = ?, MODERATOR_ID = ?, GROUP_CHAT_ID = ?, TG_NUMBER = ?, GROUP_MESSAGE_ID = ?
                WHERE NUMBER = ?
            ''', (current_time, user_id, chat_id, tg_number, None, number))
            conn.commit()
        ...
        print(f"[DEBUG] Отправлено сообщение в группу: number={number}, message_id={msg.message_id}")
        ...
        with db_module.get_db() as conn:
            ...
            cursor.execute('UPDATE numbers SET GROUP_MESSAGE_ID = ? WHERE NUMBER = ?', (msg.message_id, number))
            conn.commit()
            print(f"[DEBUG] GROUP_MESSAGE_ID сохранён: number={number}, group_message_id={msg.message_id}")
    else:
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 Главное меню", callback_data="back_to_main"))
        bot.send_message(
...
def send_verification_code(call):
    user_id = call.from_user.id
    db.update_last_activity(user_id)
    print(f"[DEBUG] send_verification_code: user_id={user_id}, callback_data={call.data}")
    try:
        parts = call.data.split("_")
        if len(parts) < 4:
            bot.answer_callback_query(call.id, "❌ Неверный формат данных!")
        ...
        with db_module.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT ID_OWNER, GROUP_MESSAGE_ID FROM numbers WHERE NUMBER = ?', (number,))
            result = cursor.fetchone()
            if not result:
                bot.answer_callback_query(call.id, "❌ Номер не найден!")
                return
            owner_id, group_message_id = result
            ...
            try:
                if group_message_id:
                    try:
                        ...
                        print(f"[DEBUG] Отредактировано сообщение в группе: number={number}, group_message_id={group_message_id}")
                    except Exception as e:
                        ...
                        with db_module.get_db() as conn:
                            ...
                            cursor.execute('UPDATE numbers SET GROUP_MESSAGE_ID = ? WHERE NUMBER = ?', (msg.message_id, number))
                            conn.commit()
                            print(f"[DEBUG] Отправлено новое сообщение в группе: number={number}, new_group_message_id={msg.message_id}")
                else:
                    msg = bot.send_message(
                        group_chat_id,
                        (
                    ...
                    with db_module.get_db() as conn:
                        ...
                        cursor.execute('UPDATE numbers SET GROUP_MESSAGE_ID = ? WHERE NUMBER = ?', (msg.message_id, number))
                        conn.commit()
                        print(f"[DEBUG] Отправлено новое сообщение в группе: number={number}, group_message_id={msg.message_id}")
                ...
                print(f"[DEBUG] Отправлено сообщение владельцу: owner_id={owner_id}, message_id={msg.message_id}")
                ...
                print(f"[DEBUG] Зарегистрирован next_step_handler: number={number}, owner_id={owner_id}, message_id={msg.message_id}")
                ...
                if e.error_code == 403 and "user is deactivated" in e.description:
                    ...
                    with db_module.get_db() as conn:
                        cursor = conn.cursor()
                        cursor.execute('UPDATE numbers SET STATUS = ?, GROUP_MESSAGE_ID = NULL WHERE NUMBER = ?', ('невалид', number))
                        conn.commit()
...
def mark_number_invalid(call):
    ...
    try:
        ...
        with db.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT ID_OWNER, MODERATOR_ID, GROUP_MESSAGE_ID FROM numbers WHERE NUMBER = ?', (number,))
            result = cursor.fetchone()
            if not result:
                bot.answer_callback_query(call.id, "❌ Номер не найден!")
                return
            owner_id, moderator_id, group_message_id = result
            ...
            try:
                cursor.execute('UPDATE numbers SET STATUS = ?, GROUP_MESSAGE_ID = NULL, VERIFICATION_CODE = NULL WHERE NUMBER = ?', ('невалид', number))
                conn.commit()
        ...
        # Очищаем button_contexts
        for key, context in list(button_contexts.items()):
            if context["number"] == number and context["user_id"] == owner_id:
                del button_contexts[key]
                print(f"[DEBUG] Удалён button_contexts key {key} для номера {number}")
...
def process_verification_code_input(message, number, moderator_id, group_chat_id, original_chat_id, original_message_id, tg_number):
    try:
        user_id = message.from_user.id
        db.update_last_activity(user_id)
        print(f"[DEBUG] process_verification_code_input: number={number}, user_id={user_id}, original_chat_id={original_chat_id}, original_message_id={original_message_id}")

        # Проверка существования и статуса номера
        with db.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT ID_OWNER, STATUS FROM numbers WHERE NUMBER = ?', (number,))
            result = cursor.fetchone()
            if not result:
                try:
            ...
            if status == 'невалид':
                ...
                bot.send_message(
                    ...
                    (
                        ...
                        "━━━━━━━━━━━━━━━━━━━━\n"
                        f"📞 <b>Номер:</b> <code>{number}</code>\n"
                        "❌ <i>Номер помечен как невалидный.</i>\n"
                        "━━━━━━━━━━━━━━━━━━━━"
                    ),
                    reply_markup=markup,
                    parse_mode='HTML'
        ...
        if not message.reply_to_message or \
           ...
           message.reply_to_message.message_id != original_message_id:
            ...
            markup.add(types.InlineKeyboardButton("❌ Не валид", callback_data=f"mark_invalid_{number}_{group_chat_id}_{tg_number}"))
            # Проверяем статус перед отправкой сообщения
            with db.get_db() as conn:
                cursor = conn.cursor()
                cursor.execute('SELECT STATUS FROM numbers WHERE NUMBER = ?', (number,))
                result = cursor.fetchone()
                if result and result[0] == 'невалид':
                    bot.send_message(
                        message.chat.id,
                        (
                            "📌 <b>Запрос кода отменён</b>\n"
        ...
        print(f"[DEBUG] Получен код: {user_input} для номера {number}")
        ...
        if not re.match(r'^\d{5}$', user_input):
            ...
            try:
                ...
                # ВАЖНО: используем register_next_step_handler_by_chat_id и передаём тот же original_message_id
                bot.register_next_step_handler_by_chat_id(
                    ...
                    group_chat_id,
                    original_chat_id,
                    original_message_id,  # тот же ID, так как мы редактировали, а не пересоздавали
                    tg_number
                )
            except Exception as e:
                print(f"[ERROR] Не удалось отредактировать сообщение для номера {number}: {e}")
                ...
                original_message_id = msg.message_id  # обновляем ID
                bot.register_next_step_handler(
                    msg,
                    process_verification_code_input,
                    number,
                    ...
                    group_chat_id,
                    original_chat_id,
                    original_message_id,  # уже новый ID
                    tg_number
                )
            return
        ...
        with db.get_db() as conn:
            ...
            conn.commit()
            print(f"[DEBUG] Код {user_input} сохранён для номера {number}")
        ...
        try:
            ...
            print(f"[DEBUG] Отредактировано сообщение для пользователя: number={number}, message_id={original_message_id}")
        except Exception as e:
            print(f"[ERROR] Не удалось отредактировать сообщение для номера {number}: {e}")
            ...
            print(f"[DEBUG] Отправлено новое сообщение для пользователя: number={number}, new_message_id={original_message_id}")
        ...
        print(f"[DEBUG] Обновлён confirmation_messages: number={number}, user_id={user_id}, message_id={original_message_id}")

        # Чистим запросы
        active_code_requests.pop(user_id, {}).pop(number, None)
        print(f"[DEBUG] Очищен active_code_requests для номера {number}, user_id={user_id}")
...
def confirm_code(call):
    try:    
        ...
        with db_module.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT MODERATOR_ID, GROUP_CHAT_ID, ID_OWNER, fa, GROUP_MESSAGE_ID,
            ...
            if not result:
                bot.answer_callback_query(call.id, "❌ Номер не найден!")
                return
            moderator_id, stored_chat_id, owner_id, fa_code, group_message_id, is_afk = result
        ...
        if moderator_id:
            ...
            try:
                ...
                cursor.execute('UPDATE numbers SET GROUP_MESSAGE_ID = ? WHERE NUMBER = ?', (message.message_id, number))
                conn.commit()
            except telebot.apihelper.ApiTelegramException as e:
                print(f"[ERROR] Не удалось отправить сообщение в группу {group_chat_id}: {e}")
                try:
        ...
        print(f"[ERROR] Ошибка в confirm_code: {e}")
...
def handle_button_context(call):
    ...
    try:
        ...
        with db_module.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT GROUP_MESSAGE_ID, fa FROM numbers WHERE NUMBER = ?', (number,))
            result = cursor.fetchone()
            if not result:
                bot.answer_callback_query(call.id, "❌ Номер не найден!")
                return
            group_message_id, two_fa_value = result
            two_fa_value = two_fa_value if two_fa_value else "не установлен"
            ...
            try:
                ...
                bot.edit_message_text(
                    (
                        ...
                        f"🔑 <b>2FA:</b> <code>{two_fa_value}</code>\n"
                        "━━━━━━━━━━━━━━━━━━━━\n"
                        "❓ Встал ли номер?"
                    ),
                    chat_id=group_chat_id,
...
def back_to_confirm(call):
    try:
        ...
        with db_module.get_db() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT ID_OWNER, VERIFICATION_CODE, TAKE_DATE, TG_NUMBER, fa, GROUP_MESSAGE_ID, (SELECT IS_AFK FROM users WHERE ID = numbers.ID_OWNER) AS is_afk FROM numbers WHERE NUMBER = ?', (number,))
            result = cursor.fetchone()
            ...
            owner_id, code, take_date, tg_number, fa_code, group_message_id, is_afk = result
            if not tg_number:
                tg_number = 1
            ...
            if is_afk:
                ...
                # Очищаем данные номера из confirmation_messages
                confirmation_messages.pop(f"{number}_{owner_id}", None)
                return
            ...
            if code and take_date != "0":
                ...
                # Обновляем GROUP_MESSAGE_ID в базе
                with db_module.get_db() as conn:
                    cursor = conn.cursor()
                    cursor.execute('UPDATE numbers SET GROUP_MESSAGE_ID = ? WHERE NUMBER = ?', (msg.message_id, number))
                    conn.commit()
...
def toggle_afk(call):
    ...
    try:
        # Отправляем уведомление только если пользователь не админ и не модератор
        is_admin = user_id in config.ADMINS_ID
        is_moderator = db_module.is_moderator(user_id)
