...
def process_appeal_number(message):
    ...
    if not row:
        bot.send_message(message.chat.id, f"❌ Номер {number} не найден в системе.")
        return

    status = row[0]
    if status != "невалид":
        bot.send_message(message.chat.id, f"❌ Номер {number} не активен (статус: {status})!")
        return
    ...
    bot.send_message(message.chat.id, f"✅ Апелляция по номеру {number} отправлена на рассмотрение.")
